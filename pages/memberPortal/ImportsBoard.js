...
class ImportsBoard {
  constructor(page) {
    ...
    this.importLabel = page.locator('button.btn btn-secondary.dropdown-toggle:text(" Enquiries ")')
  ...
  async validateImportLabel() {
    const labelText = await this.importLabel.textContent()
    expect(labelText.trim()).toEqual(data.importsBoard.label)
  }

  async validateEnrichmentErrorsImportDashboard() {
  ...
  async validateActivityErrorsImportDashboard() {
    await this.reportLabel.click()
    expect(this.dropdownMenu).toBeVisible(true)
  ...
  async validateOtherErrorsImportDashboard() {
    await this.reportLabel.click()
    expect(this.dropdownMenu).toBeVisible(true)
...
exports.ImportsBoard = ImportsBoard